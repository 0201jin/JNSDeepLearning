...
void DeepLSTM::Train_M2O(vector<vector<double>> _InputData, vector<double> _TrainData)
{
	...
	for (int i = 0; i < _InputData.size(); ++i)
	{
		...
		queue<double> TrainData = m_vNeuron[m_vNeuron.size() - 1].Train_Y(m_vNeuron[m_vNeuron.size() - 1].GetLastInput(), _TrainData[i], 0.00025, &m, &v);

		for (int j = m_vNeuron.size() - 2; j >= 0; --j)
		{
			TrainData = m_vNeuron[j].Train_H(m_vNeuron[j].GetLastInput(), TrainData, 0.00025, &m, &v);
		}
	}
}
